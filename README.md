...
* **Markdown/Text:** Fixed-size chunking with a maximum of 2000 characters.
* **Python Code:** Intelligent segmentation that attempts to keep functions and classes intact while respecting the character limit.

### Retrieval Method
The primary retrieval mechanism is **BM25** (via the `bm25s` library), chosen for its high performance on technical keywords and code symbols compared to standard TF-IDF.
...
Initial tests showed a **Recall@5 of 87%** on documentation datasets, exceeding the minimum requirements. Cold start latency is kept under 60 seconds by caching the index on disk.
...
* **AI Use:** AI was used to understand key concepts, learn librairies                  with short tutos... All the code was review and in compliance with **flake8/mypy**.